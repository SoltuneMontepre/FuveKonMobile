...
func (s *DealerService) JoinDealerBooth(userID string, boothCode string) (*responses.DealerBoothDetailResponse, error) {
	...
	activeStaff, err := s.repos.Dealer.CountActiveStaffByBooth(booth.Id.String())
	if err != nil {
		return nil, errors.New("failed to check booth capacity")
	}
	if activeStaff >= maxDealerBoothStaff {
		return nil, fmt.Errorf("dealer booth has reached the maximum number of staff (%d)", maxDealerBoothStaff)
	}
