...
func (s *TicketService) GetAllTiers(ctx context.Context, showActiveForAdmin bool) ([]responses.TicketTierResponse, error) {
	...
	// Public listing should never expose stock.
	out := mappers.MapTicketTiersToResponse(tiers, false)
...
func (s *TicketService) GetAllTiersForAdmin(ctx context.Context) ([]responses.TicketTierResponse, error) {
	...
	return mappers.MapTicketTiersToResponse(tiers, true), nil
}
...
func (s *TicketService) GetTierByID(ctx context.Context, tierID string, showActiveForAdmin bool) (*responses.TicketTierResponse, error) {
	...
	// Public tier details should never expose stock.
...
func (s *TicketService) CreateTierForAdmin(ctx context.Context, req *requests.CreateTicketTierRequest) (*responses.TicketTierResponse, error) {
	...
	return mappers.MapTicketTierToResponse(created, true), nil
}
...
func (s *TicketService) UpdateTierForAdmin(ctx context.Context, tierID string, req *requests.UpdateTicketTierRequest) (*responses.TicketTierResponse, error) {
	...
	if len(updates) == 0 {
		...
		return mappers.MapTicketTierToResponse(tier, true), nil
	}
...
// UpdateTshirtSize sets the merch T-shirt size on the user's non-denied ticket.
func (s *TicketService) UpdateTshirtSize(ctx context.Context, userID string, req *requests.UpdateTshirtSizeRequest) (*responses.UserTicketResponse, error) {
	uid, err := uuid.Parse(userID)
	if err != nil {
		return nil, ErrInvalidUserID
	}
	...
	ticket, err := s.repos.Ticket.UpdateTshirtSize(ctx, existingTicket.Id, uid, req.TshirtSize)
	if err != nil {
		return nil, err
	}
...
// ResendTicketQREmail re-sends the approved-ticket QR email to the holder.
// Valid for tickets that have been issued — i.e. status `approved` or `admin_granted`.
// The email itself reuses MailService.SendTicketApprovedWithQREmail so the template/content
// is identical to the email originally sent on approval.
func (s *TicketService) ResendTicketQREmail(ctx context.Context, ticketID string) error {
	tid, err := uuid.Parse(ticketID)
	if err != nil {
		return ErrInvalidTicketID
	}

	ticket, err := s.repos.Ticket.GetUserTicketByID(ctx, tid)
	if err != nil {
		return err
	}
	...
	tierName := ticket.Ticket.TicketName
	return s.mail.SendTicketApprovedWithQREmail(
		ctx,
		fromEmail,
		ticket.User.Email,
...
// GetTicketSalesTimeline returns ticket sales count grouped by granularity between from and to (admin).
func (s *TicketService) GetTicketSalesTimeline(ctx context.Context, from, to time.Time, granularity string) ([]responses.SalesTimelineResponse, error) {
	items, err := s.repos.Ticket.GetTicketSalesTimeline(ctx, from, to, granularity)
	if err != nil {
		return nil, err
	}
	out := make([]responses.SalesTimelineResponse, len(items))
	for i := range items {
		out[i] = responses.SalesTimelineResponse{Date: items[i].Date, Count: items[i].Count}
	}
	return out, nil
}

// GetTicketRevenue returns total revenue and optional revenue timeline (admin).
func (s *TicketService) GetTicketRevenue(ctx context.Context, from, to time.Time, granularity string) (*responses.RevenueResponse, error) {
	...
	if !from.IsZero() && !to.IsZero() {
		items, err := s.repos.Ticket.GetTicketRevenueTimeline(ctx, from, to, granularity)
		if err != nil {
			return nil, err
		}
		resp.ByDay = make([]responses.RevenueByDayResponse, len(items))
		for i := range items {
			rev, _ := items[i].Revenue.Float64()
			resp.ByDay[i] = responses.RevenueByDayResponse{Date: items[i].Date, Revenue: rev}
		}
	}
	return resp, nil
}
...
func (s *TicketService) UpdateTicketForAdmin(ctx context.Context, ticketID, staffID string, req *requests.UpdateTicketForAdminRequest) (*responses.UserTicketResponse, error) {
	...
	if req.TshirtSize != nil {
		updates["tshirt_size"] = *req.TshirtSize
	}
