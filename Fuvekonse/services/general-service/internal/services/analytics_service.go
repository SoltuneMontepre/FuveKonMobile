...
// TimelineRange holds a parsed from/to range with its granularity.
...
func (s *AnalyticsService) GetDashboard(ctx context.Context, timeline, revenue TimelineRange) (*analytics.DashboardResponse, error) {
	...
	go func() {
		defer wg.Done()
		items, err := s.ticket.GetTicketSalesTimeline(ctx, timeline.From, timeline.To, timeline.Granularity)
		mu.Lock()
		if err != nil {
			timelineErr = err
		} else {
	...
	go func() {
		defer wg.Done()
		rev, err := s.ticket.GetTicketRevenue(ctx, revenue.From, revenue.To, revenue.Granularity)
		mu.Lock()
		if err != nil {
			revenueErr = err
		} else {
