...
// UpdateTshirtSize godoc
// @Summary Update T-shirt size on my ticket
// @Description Set merch T-shirt size (XS, S, M, L, XL, XXL, 3XL). Empty string clears the size. Not allowed for denied tickets. When queue is enabled, request is queued (202).
// @Tags tickets
// @Accept json
// @Produce json
// @Security BearerAuth
// @Param request body requests.UpdateTshirtSizeRequest true "T-shirt size"
// @Success 200 "T-shirt size updated"
// @Success 202 "Request queued for processing"
// @Failure 400 "Invalid request"
// @Failure 401 "Unauthorized"
// @Failure 404 "No ticket found"
// @Failure 409 "Ticket is denied"
// @Failure 500 "Internal server error"
// @Router /tickets/me/tshirt-size [patch]
func (h *TicketHandler) UpdateTshirtSize(c *gin.Context) {
	ctx := c.Request.Context()
	userID, exists := c.Get("user_id")
	if !exists {
		utils.RespondUnauthorized(c, "User ID not found in token")
	...
	var req requests.UpdateTshirtSizeRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		utils.RespondValidationError(c, err.Error())
		return
	}
	...
	if h.queue != nil {
		if err := h.queue.PublishTicketJob(ctx, &queue.TicketJobMessage{
			Action:     queue.ActionUpdateTshirtSize,
			UserID:     userID.(string),
			TshirtSize: req.TshirtSize,
		}); err != nil {
			utils.RespondInternalServerError(c, "Failed to queue T-shirt size update")
			return
		}
		utils.RespondAccepted(c, "T-shirt size update queued for processing.")
		return
	}

	ticket, err := h.services.Ticket.UpdateTshirtSize(ctx, userID.(string), &req)
	if err != nil {
		switch {
		case errors.Is(err, services.ErrNoTicketFound):
			utils.RespondNotFound(c, "No ticket found")
		case errors.Is(err, repositories.ErrInvalidTicketStatus):
			utils.RespondError(c, 409, "INVALID_STATUS", "T-shirt size cannot be set on a denied ticket")
		default:
			utils.RespondInternalServerError(c, "Failed to update T-shirt size")
		}
		return
	}

	utils.RespondSuccess(c, ticket, "T-shirt size updated successfully")
}
...
// ResendTicketQREmail godoc
// @Summary Resend ticket QR email (admin/staff)
// @Description Re-sends the approved-ticket QR email to the holder. Valid for tickets in "approved" or "admin_granted" status. Reuses the same template/content as the email originally sent on approval.
// @Tags admin-tickets
// @Produce json
// @Security BearerAuth
// @Param id path string true "Ticket ID" format(uuid)
// @Success 200 "QR email resent"
// @Failure 400 "Invalid ticket ID"
// @Failure 401 "Unauthorized"
// @Failure 403 "Forbidden"
// @Failure 404 "Ticket not found"
// @Failure 409 "Ticket is not in an issued status (approved or admin_granted)"
// @Failure 502 "Mail provider failed"
// @Router /admin/tickets/{id}/resend-qr [post]
func (h *TicketHandler) ResendTicketQREmail(c *gin.Context) {
	ctx := c.Request.Context()
	ticketID := c.Param("id")
	if ticketID == "" {
		utils.RespondBadRequest(c, "Ticket ID is required")
	...
	if err := h.services.Ticket.ResendTicketQREmail(ctx, ticketID); err != nil {
		switch {
		case errors.Is(err, services.ErrInvalidTicketID):
			utils.RespondBadRequest(c, "Invalid ticket ID format")
		case errors.Is(err, repositories.ErrTicketNotFound):
			utils.RespondNotFound(c, "Ticket not found")
		case errors.Is(err, services.ErrInvalidTicketStatus):
			utils.RespondError(c, 409, "INVALID_STATUS", "Ticket must be approved or admin-granted to resend QR email")
		case errors.Is(err, services.ErrMailNotConfigured):
			utils.RespondError(c, 502, "MAIL_SEND_FAILED", "Mail provider is not configured")
		default:
			log.Printf("Failed to resend ticket QR email for %s: %v", ticketID, err)
			utils.RespondError(c, 502, "MAIL_SEND_FAILED", "Failed to resend QR email")
		}
		return
	}

	utils.RespondSuccess[any](c, nil, "QR email resent")
}
...
// @Description Get ticket sales count grouped by granularity between from and to
// @Tags admin-tickets
// @Accept json
// @Produce json
// @Security BearerAuth
// @Param from query string false "Timeline start (RFC3339)"
// @Param to query string false "Timeline end (RFC3339)"
// @Param granularity query string false "Grouping: day, hour, or minute" default(day) Enums(day,hour,minute)
...
func (h *TicketHandler) GetTicketSalesTimeline(c *gin.Context) {
	...
	granularity := "day"
	if v := c.Query("granularity"); v == "hour" || v == "minute" {
		granularity = v
	}
	...
	timeline, err := h.services.Ticket.GetTicketSalesTimeline(ctx, from, to, granularity)
	if err != nil {
...
// @Description Get total revenue and optional revenue timeline. Revenue = sum of tier price for all non-denied tickets.
// @Tags admin-tickets
// @Accept json
// @Produce json
// @Security BearerAuth
...
func (h *TicketHandler) GetTicketRevenue(c *gin.Context) {
	...
	revenue, err := h.services.Ticket.GetTicketRevenue(ctx, from, to, granularity)
	if err != nil {
