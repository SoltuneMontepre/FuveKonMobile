...
func (h *TicketHandler) ProcessTicketJob(c *gin.Context) {
	...
	case queue.ActionUpdateTshirtSize:
		req := &requests.UpdateTshirtSizeRequest{TshirtSize: msg.TshirtSize}
		ticket, err := h.services.Ticket.UpdateTshirtSize(ctx, msg.UserID, req)
		if err != nil {
			respondTicketJobError(c, err)
			return
		}
		utils.RespondSuccess(c, ticket, "T-shirt size update processed")
