...
func AutoMigrate(db *gorm.DB) error {
	...
	// Ensure tshirt_size exists on user_tickets
	if err := ensureUserTicketsTshirtSizeColumn(db); err != nil {
		return fmt.Errorf("failed to ensure user_tickets.tshirt_size column: %w", err)
	}
...
// ensureUserTicketsTshirtSizeColumn adds tshirt_size to user_tickets if missing.
func ensureUserTicketsTshirtSizeColumn(db *gorm.DB) error {
	migrator := db.Migrator()
	if migrator.HasTable("user_tickets") && !migrator.HasColumn("user_tickets", "tshirt_size") {
		if err := migrator.AddColumn(&models.UserTicket{}, "TshirtSize"); err != nil {
			return err
		}
		log.Println("Added user_tickets.tshirt_size column (migration)")
	}
	return nil
}
