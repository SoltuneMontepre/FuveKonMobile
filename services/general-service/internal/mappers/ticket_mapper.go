...
func MapTicketTierToResponse(tier *models.TicketTier, includeStock bool) *responses.TicketTierResponse {
...
func MapTicketTiersToResponse(tiers []models.TicketTier, includeStock bool) []responses.TicketTierResponse {
	result := make([]responses.TicketTierResponse, len(tiers))
	for i, tier := range tiers {
		result[i] = *MapTicketTierToResponse(&tier, includeStock)
	}
	return result
}
...
func MapUserTicketToResponse(ticket *models.UserTicket, includeUser bool) *responses.UserTicketResponse {
	...
	if ticket.Ticket.Id != [16]byte{} {
		response.Tier = MapTicketTierToResponse(&ticket.Ticket, includeUser)
	}
