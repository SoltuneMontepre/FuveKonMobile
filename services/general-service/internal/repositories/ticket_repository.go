...
// UpdateTshirtSize sets merch T-shirt size on the user's ticket (denied tickets cannot be updated).
func (r *TicketRepository) UpdateTshirtSize(ctx context.Context, ticketID, userID uuid.UUID, tshirtSize string) (*models.UserTicket, error) {
	var ticket models.UserTicket
...
// SalesTimelineItem holds a timestamp bucket and count for the timeline.
type SalesTimelineItem struct {
	Date  string
	Count int64
}

// truncFormatForGranularity returns the PostgreSQL date_trunc precision and
// to_char format string for a given granularity.
func truncFormatForGranularity(granularity string) (precision, format string) {
	switch granularity {
	case "hour":
		return "hour", "YYYY-MM-DD\"T\"HH24:MI:SS"
	case "minute":
		return "minute", "YYYY-MM-DD\"T\"HH24:MI:SS"
	default:
		return "day", "YYYY-MM-DD"
	}
}

// GetTicketSalesTimeline returns ticket sales count grouped by the given
// granularity ("day", "hour", or "minute") between from and to.
func (r *TicketRepository) GetTicketSalesTimeline(ctx context.Context, from, to time.Time, granularity string) ([]SalesTimelineItem, error) {
	truncPrecision, dateFormat := truncFormatForGranularity(granularity)

	var result []SalesTimelineItem
	err := r.db.WithContext(ctx).Raw(`
		SELECT to_char(date_trunc('`+truncPrecision+`', created_at), '`+dateFormat+`') AS date,
		       COUNT(*)::bigint AS count
		FROM user_tickets
		WHERE is_deleted = ?
		  AND created_at >= ? AND created_at < ?
		GROUP BY date_trunc('`+truncPrecision+`', created_at)
		ORDER BY date ASC
	`, false, from, to).Scan(&result).Error
	if err != nil {
		return nil, err
	}
	return result, nil
...
// RevenueTimelineItem holds a timestamp bucket and revenue for the timeline.
...
// GetTicketRevenueTimeline returns revenue grouped by the given granularity between from and to.
func (r *TicketRepository) GetTicketRevenueTimeline(ctx context.Context, from, to time.Time, granularity string) ([]RevenueTimelineItem, error) {
	truncPrecision, dateFormat := truncFormatForGranularity(granularity)

	var result []RevenueTimelineItem
	err := r.db.WithContext(ctx).Raw(`
		SELECT to_char(date_trunc('`+truncPrecision+`', ut.created_at), '`+dateFormat+`') AS date,
		       COALESCE(SUM(t.price), 0) AS revenue
		FROM user_tickets ut
		INNER JOIN ticket_tiers t ON ut.ticket_id = t.id AND t.is_deleted = ?
		WHERE ut.is_deleted = ? AND ut.status != ?
		  AND ut.created_at >= ? AND ut.created_at < ?
		GROUP BY date_trunc('`+truncPrecision+`', ut.created_at)
		ORDER BY date ASC
	`, false, false, models.TicketStatusDenied, from, to).Scan(&result).Error
	if err != nil {
		return nil, err
	}
	return result, nil
