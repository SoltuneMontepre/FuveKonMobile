...
// CountActiveStaffByBooth returns the number of non-deleted staff records for a booth.
func (r *DealerRepository) CountActiveStaffByBooth(boothID string) (int64, error) {
	var count int64
	err := r.db.Model(&models.UserDealerStaff{}).
		Where("booth_id = ? AND is_deleted = ?", boothID, false).
		Count(&count).Error
	return count, err
}
