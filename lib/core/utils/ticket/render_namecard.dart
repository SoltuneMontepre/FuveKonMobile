...
class RenderNamecardOptions {
  ...
  static const _tierTemplatePaths = {
...
class NamecardRenderer {
  ...
  Future<Uint8List> renderPng(RenderNamecardOptions options) async {
    ...
    final templatePath =
        RenderNamecardOptions._tierTemplatePaths[tier] ??
        RenderNamecardOptions._tierTemplatePaths[1]!;
    final templateUrl = '${AppConfig.webBaseUrl}$templatePath';

    final templateImage = await _loadImage(templateUrl);
    ...
    if (options.avatarUrl != null && options.avatarUrl!.isNotEmpty) {
      final resolved = S3Url.isS3Url(options.avatarUrl!)
          ...
          : options.avatarUrl!;
      avatarImage = await _loadImage(resolved);
    }
    ...
    if (labels.isNotEmpty) {
      final builder = ui.ParagraphBuilder(
        ui.ParagraphStyle(fontSize: labelFontPx, fontWeight: FontWeight.w600),
      )
